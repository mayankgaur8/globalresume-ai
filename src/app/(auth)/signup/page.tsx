...
const HIGHLIGHTS = [
  {
    ...
    desc: "Generate bullet points and summaries in seconds",
  },
  {
    icon: <Globe2 className="h-5 w-5 text-blue-400" />,
    title: "25+ languages",
    desc: "Localized resumes for Germany, France, Japan & more",
  },
  {
    icon: <Shield className="h-5 w-5 text-blue-400" />,
    title: "ATS-optimized",
    desc: "Pass Applicant Tracking Systems at top companies",
  },
]
        ...
        <div className="relative z-10 space-y-8">
          <div>
            ...
            <h2 className="text-4xl font-bold text-white leading-tight">
              Join 50,000+<br />career builders
            </h2>
          ...
          <div className="space-y-4">
            {HIGHLIGHTS.map((h) => (
              <div key={h.title} className="flex items-start gap-4 bg-white/5 rounded-xl p-4 border border-white/10">
                <div className="shrink-0 h-10 w-10 rounded-lg bg-blue-500/20 flex items-center justify-center">
                  {h.icon}
                </div>
                <div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex -space-x-2">
              {["A", "B", "C", "D"].map((l, i) => (
                <div
                  key={l}
                  className="h-8 w-8 rounded-full border-2 border-slate-900 flex items-center justify-center text-white text-xs font-bold"
                  style={{
            ...
            <p className="text-slate-400 text-sm">
              <span className="text-white font-semibold">2,400+</span> resumes created this week
            </p>
          </div>
        </div>

        <div className="relative z-10 text-slate-500 text-xs">
          &copy; {new Date().getFullYear()} GlobalResumeAI. All rights reserved.
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-12 bg-white">
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={onSubmit} className="space-y-5">
            ...
            <Button
              type="submit"
              className="w-full h-11 bg-blue-600 hover:bg-blue-700 text-white font-semibold shadow-sm shadow-blue-600/25"
              disabled={busy}
            >
              {isLoading ? (
                <>
